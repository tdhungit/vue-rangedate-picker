<template>
    <div class="calendar-root">
        <div class="input-date" @click="toggleCalendar()">{{getActiveTitle()}}</div>
        <div class="calendar" :class="{'calendar-mobile ': isCompact, 'calendar-right-to-left': isRighttoLeft}"
             v-if="isOpen">
            ...
            <div class="calendar-range" :class="{'calendar-range-mobile ': isCompact}" v-if="!showMonth || !isCompact">
                <ul class="calendar_preset">
                    <li
                        ...
                        <input type="number"
                               style="width: 45px"
                               v-if="item.name == 'lastNDays'"
                               v-model="ndays"
                               @change="changeNDays()">
                    </li>
                    <li>
                        <button type="button" class="calendar-btn-apply" @click="setDateValue()">
                            {{captions.ok_button}}
                        </button>
                    </li>
                    <li>
                        <button type="button" class="calendar-btn-clear" @click="clearDate()">Clear</button>
                    </li>
                </ul>
            </div>
